...
library(car)
#library(lm_robust)
data = read.csv('C:/Users/Morg/Downloads/data_analysis_round_4.csv')
```

```{r}
data$years_between_raw = data$years_between
data$years_between = abs(data$years_between)
```

```{r}
data%>%group_by(train_year,years_between_raw)%>%summarize(auc=mean(auc))%>%ggplot(aes(years_between_raw,auc))+#,color=model))
  geom_point()+geom_line()+#geom_smooth(se=FALSE)+
  facet_wrap(~train_year,scales = 'free')
  #geom_smooth(method="lm",se=FALSE)

data%>%group_by(train_year,years_between_raw)%>%summarize(ll=mean(ll))%>%ggplot(aes(years_between_raw,ll))+#,color=model))
  geom_point()+geom_line()+#geom_smooth(se=FALSE)+
  facet_wrap(~train_year,scales = 'free')

data%>%group_by(train_year,years_between_raw)%>%summarize(f1=mean(f1))%>%ggplot(aes(years_between_raw,f1))+#,color=model))
  geom_point()+geom_line()+#geom_smooth(se=FALSE)+
  facet_wrap(~train_year,scales = 'free')
```
...
```{r}
train_tib = data%>%group_by(train_year,years_between)%>%summarize(auc_m=mean(auc),auc_low=t.test(auc)$conf.int[1], auc_hi=t.test(auc)$conf.int[2],ll_m=mean(ll),ll_low=t.test(ll)$conf.int[1],ll_hi=t.test(ll)$conf.int[2],f1_m=mean(f1),f1_low=t.test(f1)$conf.int[1],f1_hi=t.test(f1)$conf.int[2])
```


```{r}
ggplot(train_tib, aes(x=years_between,y=auc_m))+
  geom_point(aes(color=train_year))+geom_line(aes(color=train_year))+
  geom_errorbar(aes(ymin=auc_low,ymax=auc_hi,width=.2,color=train_year))+
  facet_wrap(~train_year,scales='free')+
  #ggh4x::facet_manual(~train_year,scales="free",design=a_des)+
  theme(legend.position='none')+
  labs(y='AUC',x='Years between Training & Evaluation Year')
...
ggplot(train_tib, aes(x=years_between,y=ll_m))+
  geom_point(aes(color=train_year))+geom_line(aes(color=train_year))+
  geom_errorbar(aes(ymin=ll_low,ymax=ll_hi,width=.2,color=train_year))+
  facet_wrap(~train_year,scales='free')+
  #ggh4x::facet_manual(~train_year,scales="free",design=a_des)+
  theme(legend.position='none')+
  labs(y='Log Loss',x='Years between Training & Evaluation Year')
...
ggplot(train_tib, aes(x=years_between,y=f1_m))+
  geom_point(aes(color=train_year))+geom_line(aes(color=train_year))+
  geom_errorbar(aes(ymin=f1_low,ymax=f1_hi,width=.2,color=train_year))+
  facet_wrap(~train_year,scales='free')+
  #ggh4x::facet_manual(~train_year,scales="free",design=a_des)+
  theme(legend.position='none')+
  labs(y='F1 Score',x='Years between Training & Evaluation Year')
...
```{r}
aucmod = lm(auc~years_between*model+train_year*years_between+eval_year,data=data)
summary(aucmod)
coeftest(aucmod,vcov.=vcovHC(aucmod, type='HC3'))
vif(aucmod,type='predictor')
...
```{r}
llmod = lm(ll~years_between*model+train_year*years_between+eval_year,data=data)
summary(llmod)
coeftest(llmod,vcov.=vcovHC(llmod, type='HC3'))
vif(llmod,type='predictor')

plot(llmod)
```

```{r}
f1mod = lm(f1~years_between*model+train_year*years_between+eval_year,data=data)
summary(f1mod)
coeftest(f1mod,vcov.=vcovHC(f1mod, type='HC3'))
vif(f1mod,type='predictor')
...
<!-- ```{r} -->
<!-- bc = boxcox(data$f1~data$years_between*data$train_year+data$years_between*data$model+data$eval_year, lambda = seq(-10,10,1/20)) -->
<!-- bc$x[which.max(bc$y)] -->
<!-- ``` -->
...
<!-- ggplot(all_sakt, aes(bc_transform(auc,0),fill=model))+geom_histogram() -->


<!-- bcmod = lm(bc_transform(auc,-2.8)~model*years_between+train_year*years_between+eval_year,data=data) -->
<!-- summary(bcmod) -->


<!-- sakt_mod = lm(bc_transform(auc, 0.)~years_between*model,data=all_sakt) -->
<!-- summary(sakt_mod) -->
<!-- plot(bcmod) -->
<!-- coeftest(sakt_mod,vcov.=vcovHC) -->
<!-- plot(sakt_mod,which=c(1,3)) -->
