...
class SAKTModel(tf.keras.Model):
    def __init__(
        ...
        self.enc_embedding: tf.keras.layers.Embedding = tf.keras.layers.Embedding(input_dim=num_skills * 2, output_dim=hidden_units)
        self.pos_embedding: tf.keras.layers.Embedding = tf.keras.layers.Embedding(input_dim=num_steps, output_dim=hidden_units)
        self.multihead_attention: tf.keras.layers.MultiHeadAttention = tf.keras.layers.MultiHeadAttention(num_heads=num_heads, key_dim=hidden_units)
        self.feed_forward: tf.keras.Sequential = tf.keras.Sequential([
            tf.keras.layers.Dense(2048, activation='relu'),
            tf.keras.layers.Dense(self.hidden_units)
        ])
        self.sigmoid_w: tf.keras.layers.Dense = tf.keras.layers.Dense(num_skills, use_bias=True)
        self.dropout: tf.keras.layers.Dropout = tf.keras.layers.Dropout(rate=dropout_rate)
        self.optimizer: tf.keras.optimizers.Adam = tf.keras.optimizers.Adam(learning_rate=0.001, beta_2=0.98)
    ...
    def call(
        ...
        x: tf.Tensor = tf.cast(x, tf.int32)
        key_masks: tf.Tensor = tf.cast(tf.not_equal(x, 0), tf.float32)
        enc: tf.Tensor = self.enc_embedding(x) + self.pos_embedding(tf.range(self.num_steps - 1))
        ...
        attn_output, attention_weights = self.multihead_attention(
            enc, enc, training=training, return_attention_scores=True
        )
        attn_output += enc
        attn_output = tf.keras.layers.LayerNormalization()(attn_output)
        output: tf.Tensor = self.feed_forward(attn_output)
        output += attn_output
        output = tf.keras.layers.LayerNormalization()(output)
        output = tf.reshape(output, [-1, self.hidden_units])
        logits: tf.Tensor = self.sigmoid_w(output)
        logits = tf.reshape(logits, [-1])
        selected_logits: tf.Tensor = tf.gather(logits, target_id)
        pred: tf.Tensor = tf.sigmoid(selected_logits)
        target_correctness = tf.cast(target_correctness, tf.float32)
        loss: tf.Tensor = tf.reduce_sum(
            tf.nn.sigmoid_cross_entropy_with_logits(logits=selected_logits, labels=target_correctness)
        )
