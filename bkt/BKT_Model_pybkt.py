...
class BKTModel:
    def __init__(self, n_iter=100, verbose=True):
        ...
        self.skills = []
        self.defaults = {'order_id':'start_time','skill_name':'skill_id','user_id':'user_xid','correct':'discrete_score'}
        self.model = None
    ...
    def fit(self, data):
        ...
        self.model.fit(data=dat,defaults=self.defaults)

        auc = self.model.evaluate(data=dat,metric='auc',defaults=self.defaults)
    ...
    def eval(self, data):
        ...
        # ll = log_loss(final_y_true,final_y_pred)
        # auc = roc_auc_score(final_y_true,final_y_pred)
        auc = self.model.eval(data=data, defaults=self.defaults)
